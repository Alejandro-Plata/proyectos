...
class SimulationEngine {
    constructor(dbPath) {
        this.io = null;
        this.duracionRealJornada = 60; // minutos 
        this.jornadaInterval = this.duracionRealJornada * 60 * 1000;
        this.matchDuration = (this.jornadaInterval / 10) + (1 * 60 * 1000);
        this.isSaving = false;
    ...
    async fastForwardMatch(match) {
        ...
        for (let i = 0; i < numCards; i++) {
            ...
            match.events.push(this.generateCardEvent(match, cardTeam, isRed ? 'red' : 'yellow', true));
        }
    ...
    generateCardEvent(match, teamName, cardType, isRandomMinute = false) {
        const teamPlayers = this.db.players.filter(p => p.team_name === teamName);

        const playersWithRed = new Set();
        match.events.forEach(e => {
            if (e.team === teamName && (e.cardType === 'red' || e.isSecondYellow)) {
                playersWithRed.add(e.playerId);
            }
        });

        const availablePlayers = teamPlayers.filter(p => !playersWithRed.has(p.id));

        const offender = availablePlayers[Math.floor(Math.random() * availablePlayers.length)];

        let finalCardType = cardType;
        let isSecondYellow = false;
        let text = cardType === 'red' ? '¡Tarjeta Roja!' : 'Tarjeta Amarilla';

        if (cardType === 'yellow') {
            const hasYellow = match.events.some(e =>
                e.playerId === offender.id && e.cardType === 'yellow'
            );

            if (hasYellow) {
                finalCardType = 'red';
                isSecondYellow = true;
                text = '¡Segunda Amarilla! Expulsado';
            }
        }
        ...
        if (offender.id) {
            if (finalCardType === 'red') {
                db.query(`UPDATE players SET red_cards = red_cards + 1 WHERE id = $1`, [offender.id]).catch(() => { });
            } else {
                db.query(`UPDATE players SET yellow_cards = yellow_cards + 1 WHERE id = $1`, [offender.id]).catch(() => { });
            }
        }
        ...
        return {
            ...
            playerAvatar: offender.avatar_url,
            minute: Math.min(90, Math.max(1, minute)),
            text: text
        };
    }
    ...
    simulateMatchProgress(match) {
        ...
        if (Math.random() > 0.97) {
            ...
            const event = this.generateCardEvent(match, cardTeam, cardType);
