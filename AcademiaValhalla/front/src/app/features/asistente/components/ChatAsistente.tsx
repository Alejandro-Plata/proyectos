...
export const ChatAsistente = ({ contexto, context, mostrarEncabezado = true, esMobile = false }: PropsChatAsistente) => {
    ...
    const { messages: mensajes, status: estado, sendMessage: enviarMensaje, clearChat: limpiarChat, messagesEndRef: refFinalMensajes, distillToNote, distilling, canDistill } = useChatAsistente({ context: contextoCombinado });
    ...
    // A5 · Destila la conversación en un borrador de apunte y abre el editor
    const convertirEnApunte = async () => {
        const borrador = await distillToNote();
        if (!borrador) return;
        sessionStorage.setItem('borrador-apunte-ia', JSON.stringify(borrador));
        navigate('/dashboard/notes/create');
    };

    return (
        <div className={`relative flex flex-col h-full ${esMobile ? 'bg-slate-50 dark:bg-[#050505]' : 'bg-white dark:bg-[#0a0b0e] border border-emerald-500/15 dark:border-emerald-500/10 shadow-sm shadow-emerald-500/5'}`}>
            ...
            {mostrarEncabezado && (
                <header className="scanline-bottom shrink-0 flex items-center justify-between px-6 h-16 bg-white dark:bg-[#0a0b0e] border-b border-emerald-500/10">
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            onClick={() => setHerramientasAbiertas(true)}
                            title="Memoria y revisión de código"
                        ...
                        {canDistill && (
                            <button
                                onClick={convertirEnApunte}
                                disabled={distilling}
                                title="Convertir esta conversación en un apunte"
                                className="flex items-center gap-1.5 text-xs font-semibold text-emerald-600 dark:text-emerald-400 h-10 px-3 border border-emerald-500/30 hover:border-emerald-500/60 hover:bg-emerald-500/[0.06] transition-colors disabled:opacity-50"
                            >
                                <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M12 6.042A8.967 8.967 0 0 0 6 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 0 1 6 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 0 1 6-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0 0 18 18a8.967 8.967 0 0 0-6 2.292m0-14.25v14.25" />
                                </svg>
                                {distilling ? 'Destilando...' : 'Convertir en apunte'}
                            </button>
                        )}
                        {tieneMensajes && (
                            <button
            ...
            {herramientasAbiertas && <HerramientasIA onClose={() => setHerramientasAbiertas(false)} />}
        </div>
    );
};
