...
export default function XPRewardModal({ reward, visible, onClose }: XPRewardModalProps) {
        ...
        const ctx = gsap.context(() => {
            ...
            if (reward.leveledUp) {
                ...
                const prevPercent = Math.max(
                    0,
                    reward.progressPercent - Math.round((reward.xpGained / reward.requiredForNextLevel) * 100)
                );
                tl.fromTo(
                    progressBarRef.current,
                    { width: `${prevPercent}%` },
                    { width: `${reward.progressPercent}%`, duration: 1, ease: 'power2.out' },
                    '-=0.6'
                );
            }
        }, containerRef);
    ...
    return (
        <div ref={containerRef} className="fixed inset-0 z-[9999] flex items-center justify-center">
            ...
            >
                ...
                <h2 className={`font-mono text-[11px] uppercase tracking-[0.3em] mb-1 ${isLevelUp ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
                    {isLevelUp ? XP_MODAL_MESSAGES.LEVEL_UP_TITLE : XP_MODAL_MESSAGES.XP_GAINED_TITLE}
                </h2>
                <p className="font-mono text-slate-500 dark:text-gray-400 text-[10px] uppercase tracking-[0.15em] mb-4">
                    {isLevelUp ? XP_MODAL_MESSAGES.LEVEL_UP_SUBTITLE : XP_MODAL_MESSAGES.XP_GAINED_SUBTITLE}
                </p>
                ...
                <div ref={levelBadgeRef} className="mb-4">
                    <div
                        ...
                        className={`inline-flex items-center gap-2 px-4 py-2 ${isLevelUp ? 'bg-amber-100 border border-amber-400/60 dark:bg-amber-500/20 dark:border-amber-500/50' : 'bg-emerald-100 border border-emerald-400/60 dark:bg-emerald-500/20 dark:border-emerald-500/50'}`}
                    >
                        <svg className={`w-5 h-5 ${isLevelUp ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M14.5 17.5L3 6V3h3l11.5 11.5m0 0l-1.5 1.5m1.5-1.5L21 21m-8.5-8.5L15 11"/></svg>
                        <span className={`font-mono font-bold text-lg ${isLevelUp ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
